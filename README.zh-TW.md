...
- **大量下載搜尋引擎結果**（Google 圖片、Bing 圖片等）— 版權散落在幾千個不相關的網站，你跟其中任何一個都沒關係
- **跨整個 web 大量採集圖片** — 同樣的問題、規模更大
- **建立圖像訓練資料集** — 這有專門的工具跟授權來源、請用那些

如果你需要的是「按概念找圖」而不是「歸檔某個你已經知道的頁面」、用 [Unsplash](https://unsplash.com/developers) 或 [Pexels](https://www.pexels.com/api/) 這類有授權的 stock API，或直接用 AI 生圖。

## 使用守則

這個工具是設計給「歸檔你已經有關係的特定頁面」用 — 你的購物車、你在學習的某個設計師的 Behance project、你收藏的某個物件。請：

- 尊重各網站的 robots.txt 與服務條款
- 不要連續高頻跑批次 — 程式內建隨機等待、不要把它拿掉
- 抓下來的圖未經原店家／創作者同意不要公開散佈
- 遇到驗證碼或登入牆就停下來思考一下：自動化真的合適嗎

## 責任歸屬

我（作者）以 MIT 授權**原樣**提供這個工具。**你決定要把它指向哪些網站 — 那個決定是你的。** 我不為以下事項負責：
